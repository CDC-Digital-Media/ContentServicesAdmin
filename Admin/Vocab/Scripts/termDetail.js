...
(function ($) {
	...
	$.fn[PLUGIN_NAME] = function (options) {
		...
		function main() {
			...
			loadExistingTerms(options.valueSet.id);
			...
			$det.load("Templates/TermDetail.htm", function () {
				...
				// add language options where language != current language && alternate language value sets exists (key: ValueSet Name + language)
				var toAdd = $.grep(options.languages, function (itm) { return itm.name !== options.valueSet.language; });
				$(toAdd).each(function (i, o) {

					var valueSetForLanguage = $.grep(options.valueSets, function (itm) {
						return itm.name === options.valueSet.name && itm.languageCode === o.name
					});

					if (valueSetForLanguage.length > 0) {
						var $li = $("<li><a tabindex='-1' href='#' code='TT' language='" + o.name + "' valueSetId='" + valueSetForLanguage[0].id + "'>translates to (" + o.name + ")</a></li>")
		...
		var setupTermDetail = function () {
			...
			$det.find('.addRelationshipRow').unbind('click').click(function () {
				...
				var $tbx = $lastRow.find(".valueSearch:visible, .altValueSearch:visible");

				var lang = $tbx.attr("language");
				lang = lang ? lang : "English";

				var termName = $tbx.val();
				if (!validateTermName(termName, lang)) { return false; }
		...
		var deleteRelationship = function (oRel) {
			...
			var displayErrors = function (obj) {
				if (obj.meta.message.length > 0) {

					var errors, warnings, messages, $term;

					errors = $.grep(obj.meta.message, function (msg) { return (msg.type == "Error"); });
					warnings = $.grep(obj.meta.message, function (msg) { return (msg.type == "Warning"); });

					if (errors.length > 0) {
						messages = "<ul>";
						$(errors).each(function () { messages = messages + "<li>" + $(this)[0].userMessage + "</li>"; });
					...
					if (warnings.length > 0) {
						messages = "<ul>";
						$(warnings).each(function () { messages = messages + "<li>" + $(this)[0].userMessage + "</li>"; });
						messages = messages + "</ul>";
			...
			CDC.Admin.Lookup.deleteValueRelationship(options.term.valueId, oRel, handleUpdate, displayErrors)
		}
		...
		var saveRelationships = function () {
			...
			CDC.Admin.Lookup.addValueRelationship(options.term.valueId, oRel, handleUpdate, displayErrors)
		...
		var setupRelationshipRow = function ($det, $row, relType) {
			...
			setupTypeAhead($row.find('.valueSearch'), options.valueSet.language);
			...
			$row.find('.relationshipType ul a').click(function () {
				...
				if ($(this).attr("code") == "TT") {
					var lang = $(this).attr("language");
					var valueSetId = $(this).attr("valueSetId");
					setupAltAutosuggest($row, lang, valueSetId);
				} else {
					$row.find(".altValueSearch").hide();
					$row.find(".valueSearch").show();
				}
		...
		function loadExistingTerms(valueSetId, postProcess) {
			...
			var lang = $.grep(options.valueSets, function (itm) {
				return itm.id === eval(valueSetId);
			})[0].languageCode;

			var valueSetData, typeAheadArray = [], mapped = {};
			if (!data[lang]) {
				CDC.Admin.Lookup.getValuesForValueSet(valueSetId, function (response) {
		...
		function setupTypeAhead($field, lang) {
			$field.typeahead({
				source: data[lang].typeAheadArray,
				updater: function (term) {
					var id = data[lang].mapped[term];
		...
		function setupAltAutosuggest($row, lang, valueSetId) {
			$(".relationshipBuilder").showSpinner();
			$row.find(".valueSearch").hide();
			$row.find(".altValueSearch").remove();

			var $altAuto = $('<input type="text" class="form-control altValueSearch input-sm" language="' + lang + '" data-provide="typeahead" data-items="4" placeholder="Search for ' + lang + ' value" style="margin-top:-5px;" />');
			$row.find(".valueSearch").parent().append($altAuto);

			loadExistingTerms(valueSetId, function () {
				setupTypeAhead($altAuto, lang)
				$(".relationshipBuilder").hideSpinner();
			});
		}
			...
			$rList.each(function () {
				...
				if (relationship.type == typeCode) {
					...
					$aDelete.click(function () {
						...
						// do grep - get relationship.
						var oRel = $.grep(term.relationships, function (r, idx) {
							return relationship.type == r.type && relationship.relatedValueId == r.relatedValueId;
						})[0];

						var data = { 'relationships': [{ 'type': oRel.type, 'valueId': oRel.relatedValueId }] };
		...
		function validateTerm() {
			...
			var arr = $.map(data[options.valueSet.language].typeAheadArray, function (elem) { return elem.toLowerCase(); });
		...
		function validateRelationships(rels) {
			...
			function getRelatedByType(arr, oTerm, relType) {
				...
				$(relationShips).each(function () {
					var r = $(this)[0];
					var o = $.grep(data["English"].valueSetData, function (e, i) {
		...
		function getRelationships() {
			...
			$(".relationshipBuilderRow").each(function () {
				...
				var $tbx = $(this).find(".valueSearch:visible, .altValueSearch:visible");
				var relTermId = $tbx.attr("relatedTermId");
				var termName = "";

				var termName = $tbx.val();

				if (termName) {
					if (!validateTermName(termName, options.valueSet.language)) { return false; }
