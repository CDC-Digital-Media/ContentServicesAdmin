...
function initalize() {
	...
	switch (view) {
		...
		default:
			...
			$('#feedModal, .trail').hide();
			var filter = $("#filterPanel").mediaFilter({
				searchHandler: search,
				mediaType: "Feed,Feed - Proxy,Feed - Import,Feed - Aggregate"
			});
			break;
	}
}
...
function loadAggregateInfo(_m, $t) {
	/*
	"feedAggregates": [
		{ "id": 8, "queryString": "contentGroup=facebook&max=10" }, 
		{ "id": 9, "queryString": "contentGroup=newsroom&max=10" }, 
		{ "queryString": "contentGroup=Flickr&max=10"}
	]	
	*/

	$(_m.feedAggregates).each(function () {

		var row = $t.find(".aggSourceRow").last();
		row.find("#aggSource").val(this.queryString);

		if ($t.find(".aggSourceRow").length < $(_m.feedAggregates).length) {
			var newrow = $t.find(".aggSourceRow").first().clone();
			newrow.find(".control-label").text("");
			$t.find(".aggSourceBtnRow").before(newrow);
		} 
...
function getAggregateInfo(_m, $t) {
	...
	*/
	...
	var aggUrls = [];
	$t.find('.aggSourceRow').each(function () {
		var url = $(this).find('#aggSource').val();
		if (url !== '') {
			var jatt = { "queryString": url }
			aggUrls.push(jatt);
		}
	});
	_m.feedAggregates = aggUrls;
}

function getProxyOrImportInfo(_m, $t) {

	if (_m.mediaType == "Feed - Proxy") {
		_m.sourceUrl = $t.find("#proxyUrl").val();
	}

	if (_m.mediaType == "Feed - Import") {
		_m.sourceUrl = $t.find("#importUrl").val();
	}
...
function loadContentGroupInfo(_m, $t) {
	...
	$.ajax({
		...
		$(response.results).each(function () {
			if (selectedContentGroups.indexOf(this.valueId) == -1) {
				this.valueName = htmlEncode(this.valueName);
				this.description = htmlEncode(this.description);
	...
	function buildRow(item) {
		...
		item.valueName = htmlEncode(item.valueName);
		item.description = htmlEncode(item.description);
...
function validateResourceExists($field, isValid, next) {
	...
	$.ajax({
		...
		success: function (response) {
			...
			//	isValid = false;
			//	$field.parents('.form-group').find('.validationMsg').show().text('No content was returned for: ' + $field.val());
			//}
...
function isImportUrlValid($t) {
	...
	if (!$.trim($this.val()).length) {
		$this.parents('.form-group').find('.validationMsg').show().text('Import URL is a required field.');
		isValid = false;
	} else if (!isValidUrlFormat($this.val())) {
		$this.parents('.form-group').find('.validationMsg').show().text('Import URL is not in a valid URL format.');
		isValid = false;
	}
	return isValid;
}
