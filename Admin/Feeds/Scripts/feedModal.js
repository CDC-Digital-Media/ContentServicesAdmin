(function ($) {
	...
	$.fn[PLUGIN_NAME] = function (options) {
		...
		var main = function () {
			...
			$t.load("Templates/feedModal.htm", function () {

				$t.find(".isManaged, .isImport, .isProxy, .isAggregate").hide();

				if (_step == 6) {
					$t.find(".modal-title").text("New Feed");
					$t.find(".progress-bar, #prev, #next, #saveFeed").hide();

					// choose feed type -
					$t.find("#createAggregate").off().click(function () {
						_type = "Feed - Aggregate";
						$t.find(".isAggregate").show();
						_step = 0; main();
					});

					$t.find("#createManaged").off().click(function () {
						_type = "Feed";
						_step = 0; main();
					});

					$t.find("#createProxy").off().click(function () {
						_type = "Feed - Proxy";
						$t.find(".isProxy").show();
						_step = 0; main();
					});

					$t.find("#createImport").off().click(function () {
						_type = "Feed - Import";
						$t.find(".isImport").show();
						_step = 0; main();
					});
				}
				...
				switch(_type) {
					case "Feed - Aggregate":
						$t.find(".catHeader").text("Aggregate Sources")
						$t.find(".isAggregate").show();
						break;
					case "Feed - Import":
						$t.find(".catHeader").text("Import Information")
						$t.find(".isImport").show();
						break;
					case "Feed - Proxy":
						$t.find(".catHeader").text("Proxy Information")
						$t.find(".isProxy").show();
						break;
					case "Feed":
						$t.find(".catHeader").text("Categorization")
						$t.find(".isManaged").show();
						break;
					default:
					//default code block
				} 

				if (_m.id) { $(".modal-title").text("Edit Feed"); }
				else { $(".modal-title").text("Add Feed"); }
		...
		function saveFeed() {
			...
			getAggregateInfo(_m, $t);
			getProxyOrImportInfo(_m, $t);
			getGeneralInfo(_m, $t);
		...
		function setupEvents() {
			...
			function applyAggSrcRowDelete() {
				...
				$t.find(".aggSourceDelete").off().click(function () {
					...
					$t.find(".aggSourceRow .control-label").first().text("Source URL");
				});
		...
		function validateFeed(saveHandler) {
			...
			function validatePage0(next) {
				...
				} else if (_type == "Feed - Aggregate") {
					if (!isAggregateValid()) isValid = false;
				}
				...
				if (_type == "Feed - Import") {
					if (!isValid) { _step = 0; updateDisplay(); return; }
						...
						var $field = $t.find('#importUrl');
						validateResourceExists($field, isValid, handleNext)
					}
				} else {
					handleNext(isValid);
				}
		...
		function isAggregateValid() {
			var isValid = false;
			$t.find('.aggSourceRow').each(function () {
				if ($(this).find('#aggSource').val() !== '') { isValid = true };
			});
			if (!isValid) {
				$t.find('.aggSourceRow').first().find('.validationMsg').show().text('At least one Aggregate Source is required.');
			}
			return isValid;
		}
